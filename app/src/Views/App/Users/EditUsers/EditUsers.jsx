...
const EditUsers = () => {
  ...
  const ViewStrings = Strings.Users.EditUser;
  ...
  const fetchUser = async () => {
    return await request("get", getEndpoint(Endpoints.Users.editUser.get), {
      guid: user_guid,
    })
  ...
  const handleSubmit = () => {
    if (checkForm()) {
      request("post", getEndpoint(Endpoints.Users.editUser.update), {
        ...data,
      })
        .then(() => successNotification(ViewStrings.messages.userUpdated))
        .then(() => push(Paths[Views.users].path))
        .catch((err) => errorNotification(err.message));
